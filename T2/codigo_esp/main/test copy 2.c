...
static const uint8_t heart_measurement_ccc[2] = {0x009, 0x0};
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
        ...
        int descr_value = param->read.handle;
        ESP_LOGI(GATTS_TABLE_TAG, "Description value:%x", descr_value);
        uint16_t handle = param->add_attr_tab.num_handle;
        ESP_LOGI(GATTS_TABLE_TAG, "Handle:%x", handle);
        if (heart_rate_handle_table[IDX_CHAR_CFG_A]) {
            esp_gatt_rsp_t rsp;
            memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
            rsp.attr_value.handle = heart_rate_handle_table[IDX_CHAR_CFG_A];
